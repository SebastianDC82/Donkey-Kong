...
public class MenuInicial extends EstadoAbs {
	protected Personajes chara; //Donkey Kong character only
	public MenuInicial(Controlador gamestates) {
		super(gamestates);
	}
	...
	public void inicial() {
		chara = new Personajes(655, 360); //draws Donkey Kong
	}
	...
	public void keyPressed(int kp) {
		...
		if (kp == KeyEvent.VK_ENTER) { //if enter key pressed
			if (select == 0) { // play
				EstadoJuego.stages.push(new Nivel1(EstadoJuego));//taken to level
	...
	protected void dibujar(Graphics g) {
		...
		chara.dibujar(g); // draws the characters
		g.setColor(Color.black);
